...
impl Fragment {
    ...
    fn build_display_list_for_text_fragment(
        ...
        let adjust_thickness = |thickness: Length| -> Length {
            // Round to the nearest integer device pixel, ensuring at least one device pixel.
            Length::new((thickness.px() * dppx).round().max(1.0) / dppx)
        };
            ...
            .contains(TextDecorationLine::UNDERLINE)
        {
            ...
            rect.size.height = adjust_thickness(font_metrics.underline_size);
            ...
            .contains(TextDecorationLine::LINE_THROUGH)
        {
            ...
            rect.size.height = adjust_thickness(font_metrics.strikeout_size);
            self.build_display_list_for_text_decoration(fragment, builder, &rect, color);
        }
    }
