...
impl Fragment {
    ...
    fn build_display_list_for_text_fragment(
        ...
        let dppx = state
            ...
            .device_pixel_ratio()
            .get();
        let adjust_thickness = |thickness: Au| -> Au {
            // Round to the nearest integer device pixel, ensuring at least one device pixel.
            Au::from_f32_px((thickness.to_f32_px() * dppx).round().max(1.0) / dppx)
        };
        ...
        if text_decorations.underline {
            ...
            stacking_relative_box.size.block = adjust_thickness(metrics.underline_size);
        ...
        if text_decorations.line_through {
            ...
            stacking_relative_box.size.block = adjust_thickness(metrics.strikeout_size);
            self.build_display_list_for_text_decoration(
                state,
                &text_color,
                &stacking_relative_box,
